...
export function MobileHeader({ inNote, noteTitle, onHamburger, userName = '', userImage }: MobileHeaderProps) {
  ...
  if (inNote) {
    return (
      <header
        className="fixed top-0 left-0 right-0 z-40 bg-bg-base border-b border-border-default flex items-center px-4 gap-3"
        style={{ height: 'var(--mobile-header-total)', paddingTop: 'var(--safe-top)' }}
      >
  ...
  return (
    // Same story as the note header: the 56px bar plus whatever the status bar
    // claims, so the logo is never drawn behind the clock.
    <header
      className="fixed top-0 left-0 right-0 z-40 bg-bg-base border-b border-border-default flex items-center px-4"
      style={{ height: 'var(--mobile-header-total)', paddingTop: 'var(--safe-top)' }}
    >
